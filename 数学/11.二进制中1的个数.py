...
24ms
5632k
伪代码:1. 思想一个整数不为0，那么这个整数至少有一位是1。如果我们把这个整数减1，那么原 来处在整数最右边的1就会变为0，原来在1后面的所有的0都会变成1(如果最右边的1后面还有0的 话)。其余所有位将不会受到影响。
2. 用减1之后的数和原来的数相与运算，得到的数为原来的数最 右的1变为0的数(1的个数减1)， 3. 循环第二步，得到结果
